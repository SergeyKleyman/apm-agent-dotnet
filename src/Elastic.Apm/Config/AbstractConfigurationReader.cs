...
namespace Elastic.Apm.Config
{
	public abstract class AbstractConfigurationReader
	{
		...
		protected double ParseSpanFramesMinDurationInMilliseconds(ConfigurationKeyValue kv)
		{
			...
			{
				Logger?.Critical()
					?.LogException(e, "Exception thrown from TryParseTimeInterval which means a programming bug - " +
						"using default: {DefaultSpanFramesMinDuration}",
						DefaultValues.SpanFramesMinDuration);
				return DefaultValues.SpanFramesMinDurationInMilliseconds;
			}
		...
		protected TimeSpan ParseDiscardEventAge(ConfigurationKeyValue kv) =>
			ParsePositiveOrZeroTimeIntervalInMillisecondsImpl(kv, TimeSuffix.S,
				TimeSpan.FromMilliseconds(DefaultValues.DiscardEventAgeInMilliseconds), "DiscardEventAge");
				...
				default:
					...
					switch (defaultSuffix)
					{
						...
						case TimeSuffix.S:
							...
							break;
						default:
							throw new ArgumentException($"Unexpected TimeSuffix value: {defaultSuffix}", nameof(defaultSuffix));
					}
