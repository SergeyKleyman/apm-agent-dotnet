...
namespace Elastic.Apm.Tests.TestHelpers
{
	...
	internal class AgentTimerForTesting : IAgentTimerForTesting
	{
		...
		private readonly AgentTimer _agentTimer = new AgentTimer();
		...
		public bool IsMockTime => false;

		public AgentTimeInstant Now => _agentTimer.Now;
		...
		{
			var realNow = _agentTimer.Now;
			CatchUpMockTimerToReal(realNow);
			var leftToDelay = relativeToInstant + delay - realNow;
			await _mockAgentTimer.Delay(_mockAgentTimer.Now, leftToDelay, cancellationToken);
			await _agentTimer.Delay(realNow, leftToDelay, cancellationToken);
		}

		private void CatchUpMockTimerToReal(AgentTimeInstant realNow) => _mockAgentTimer.FastForward(realNow - _agentTimer.StartTime);
